...
?>
  ...
  <form role="form" id="form-allot-universities" action="/app/sub-centers/allot" method="POST" enctype="multipart/form-data">
    <div class="modal-body">
      <?php 
          $universities = $conn->query("SELECT Universities.ID, CONCAT(Universities.Short_Name, ' (', Universities.Vertical, ')') as Name FROM Alloted_Center_To_Counsellor LEFT JOIN Universities ON Alloted_Center_To_Counsellor.University_ID = Universities.ID WHERE Alloted_Center_To_Counsellor.Code = $center");
  ...
  <script>
    ...
    $("#form-allot-universities").on("submit", function(e){
      if($('#form-allot-universities').valid()){
        ...
        formData.append('reporting', '<?=$center?>');
        $.ajax({
            url: this.action,
            type: 'post',
            data: formData,
