...
// echo "SELECT Amount as received , Transaction_ID as art_id ,  Payments.Added_By FROM Payments WHERE Transaction_ID='" . $ledger['Transaction_ID'] . "'";
                  ...
                  // echo "<pre>"; print_r($payments_arr);
                    ...
                    $students = $conn->query("SELECT CONCAT(TRIM(CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name)), ' (', IF(Students.Unique_ID='' OR Students.Unique_ID IS NULL, RIGHT(CONCAT('000000', Students.ID), 6), Students.Unique_ID), ')') as Student_Name , Students.ID as std_ID FROM Student_Ledgers LEFT JOIN Students ON Student_Ledgers.Student_ID = Students.ID WHERE  Student_Ledgers.Transaction_ID = '" . $payments_arr['art_id'] . "'  AND Student_Ledgers.University_ID = " . $_SESSION['university_id'] . "  GROUP BY Student_ID ");
                    }
                    $student_name = array();
